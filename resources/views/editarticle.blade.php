    ...
    <div class="w-full h-full p-5">
        <div class="h-full w-full bg-gray-300 flex flex-col items-center">
            ...
            <form action="/article/update/{{ $article->id }}" method="POST"
                ...
                @csrf
                @method('PUT')
                <h1 class="uppercase mb-10 text-lg border-b border-orange-500">Editar Artigo {{ $article->name }}</h1>
                <div class="w-full flex flex-col md:flex-row justify-evenly mb-5">
                    <input type="text" class="border-0 rounded w-3/4 md:w-1/5" name="name" required
                        placeholder="Nome do Artigo" value="{{ $article->name }}">
                    <select name="category_id" class="border-0 rounded w-3/4 md:w-1/5 px-2" required>
                        <option value="{{ $article->category_id }}" class="text-gray-400">
                            @foreach ($catarticle as $catarticle)
                                {{ $catarticle->name }}
                            @endforeach
                        </option>
                        @foreach ($cats as $cat)
                ...
                <div class="w-full h-full">
                    <textarea name="content" id="content" class=" h-full">{!! $article->content !!}</textarea>
                </div>
